...
class Env:
    ...
    def __init__(
        ...
        if self._dataset:
            ...
            if self._generate_episode:
                ### Generation
                self._setup_episode_iterator_new()
    ...
    def _setup_episode_iterator_new(self):
        assert self._dataset is not None
        
        # 生成一个随机索引
        random_start_index = random.randint(0, len(self.episodes) - 1)
        ...
        # 创建从随机索引开始的 episode 迭代器
        self._episode_iterator = islice(self.episodes, random_start_index, None)
    ...
    def episode_start_time(self) -> Optional[float]:
        return self._episode_start_timeF
    ...
    def reset(self) -> Observations:
        ...
        if (
            ...
            and not self._generate_episode
        ):
    ...
    def reset_random(self) -> Observations:
        r"""Resets the environments and returns the initial observations.
